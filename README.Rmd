...
I suspsect that there are further table arrangements, but they are not clear to me at this stage, issues submitted by users and contributers should be helpful.
...
* I envision for a next version some functions for assisted construction of the schema. Those could for instance be `setCluster()`, `setHeader()` and `setVariable()`, which could be piped into one another and finally into `makeSchema()`.
