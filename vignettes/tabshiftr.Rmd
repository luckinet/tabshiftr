...
3. **Clusters**: provide (optionally) in `setCluster()`
...
4. **Identifying variables**: provide in `setIDVar()`
    ...
    - is the variable wide (i.e., its values are in several columns) ([Tab. 7](#one_wide_id), [Tab. 8](#wide_obs) & [Tab. 9](#two_wide_id))? In this case, the values will look like it's are part of the header.
...
As an example, we show here how to build a schema description for a table that has a wide identifying variable and a listed observed variable

```{r}
kable(input <- tabs2shift$listed_column_wide)
```

In this case we don't need to set clusters and can start immediately with setting the id variable `territories`, which is in the first column and otherwise tidy. Any of the setters start by default with an empty schema, in case none is provided to them from a previous setter

```{r}
schema <- setIDVar(name = "territories", columns = 1)
```

Since version 0.3.0, `tabshiftr` comes with getters that allow to debug the current schema description. To do this, however, the schema first needs to be validated. This is in order to make sure that all the generic meta-data are evaluated with the respective input. After that, a getter can be used to extract the respective information, for example the reorganised id variables with `getIDVars()`.
...
After seeing that our specification results in a meaningful output, we can continue setting the other variables `years` (tidy and in column 2) and `commodities` (spread over two columns and the values are in the first row).
...
The id variable `commodities` is visibly wide (more than one column) and its' values are not repeated four times (as for the other id variables) but only once.
Hence, this is an expected tentative output and the id variables have been specified correctly.

Next, we set the listed observed variables, which means that the column names of the observed variables are treated as if they were the values of an identifying variable (in column 4), while the values are in the columns 6 and 7 and need to be filtered by the respective variable name in column 4.
...
Table: <a name="clustNest"></a>Table 15: Clusters are grouped according to a variable of interes that should also be captured.
