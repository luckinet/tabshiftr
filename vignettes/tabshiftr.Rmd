...
2. Determine whether there are clusters, find the origin (top left cell) of each cluster and provide the required information in `setCluster()` ([Tab. 9](#clusHor), [Tab. 10](#clusId) & [Tab. 11](#clusMeas)). It is encouraged to treat a table that does not start at the topmost left cell as cluster ([Tab. 6](#implicitVar)). Follow the next steps for each cluster...
