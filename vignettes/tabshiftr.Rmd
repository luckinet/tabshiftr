...
Several of these dimensions can co-occur in one table, which might make it tricky to find the correct schema ad-hoc for some tables.
...
2) The names of measured variables are treated as if they were the levels of an identifying variable (they are listed), instead of each measured variable in its own column. This has also been called "long" table in the past.
3) The levels of one or more identifying variable are spread over several columns, instead of all identifying variables in only one column. This has also been called "wide" table in the past.
4) There is a mismatch of columns and variables, which might manifest either in that 

    * several identifying variables are stored in the same column, 
    * one identifying variable is split up into several columns,
    * variables are stored in a location that is not connected to the remaining topologically coherent chunk or
    * a variable is available only implicitly (typically the cluster ID), either as table name or somewhere in the metadata.
